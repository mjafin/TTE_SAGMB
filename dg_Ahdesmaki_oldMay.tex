...
% New abstract
% Model selection between competing models is a key consideration in the discovery of prognostic multigene signatures. It is imperative to use appropriate statistical performance measures as well as to verify biological significance of the discovered signatures to guarantee a chance of the chosen signature validating externally. Current approaches in time-to-event studies often dichotomise the follow-up times at some phase of the study to facilitate signature discovery, use only a single measure of performance such as log-rank test p-values or ... In this study we improve the prognostic signature discovery process through the application of the multivariate partial Cox model combined with the concordance index, hazard ratio of predictions, independence from available clinical covariates and biological enrichment as a measures of signature performance. The proposed framework was applied to discover prognostic multigene signatures from early breast cancer data. The partial Cox model combined with the multiple performance measures were used in both guiding the selection of the optimal panel of prognostic genes and prediction of risk within cross validation, without dichotomising the follow-up times at any stage. The signatures were successfully externally cross validated in independent datasets, yielding a hazard ratio of $2.55 [1.44, 4.51]$ for the top ranking signature. 

% New updated abstract
...
\textcolor{red}{Due to the public unavailability of the original implementation,} Partial Cox regression will be made available in R from the package \verb1PartialCox1. The c-index and many other useful functions are available in the package \verb1Hmisc1 .


\subsection{Definitions of important parameters and statistics}
The risk score used in this publication is defined as the linear combination of the signature probeset values multiplied by their corresponding partial Cox model coefficients, first subtracted by the training set probeset mean values \textcolor{red}{(see \citet{Li:04} for the Cox model and risk score definitions)}:
\begin{equation}
...
\textcolor{red}{The risk scores have by definition a sample mean of zero \citet{Li:04}, and dichotomisation of the risk scores for hazard ratio calculation is obtained via the indicator function,}
\begin{equation}
I_{y^{new}>0},
\label{eq:dichRiskScore}
\end{equation}
\textcolor{red}{which equatos to one for positive values and zero otherwise.}
